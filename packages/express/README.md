...
`auth.requireIdentity()` requires *somebody*, not a scope: the first three rows of that table, then `next()`. No tenant, no scope, no database round trip.
...
app.post(
    ...
    async (req, res, next) => {
        try {
            const tenantId = await getAuthContext(req).as!.createWorkspace(req.body.name);
...
Since those routes are refused inside SQL, **mount `auth.errorHandler()`** so the refusal is a 403 rather than a 500.
