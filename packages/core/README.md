...
| `enforceIdentity(context)` | The first two branches alone — 401 with no credential, 403 for a credential that maps to no identity — with no tenant, no scope and no round trip. For routes where the kit's SQL anchors authority on something the request doesn't name: `createWorkspace` needs no scope at any tenant, and `revokeBinding`, `updateRole`, `addRoleScope`, `removeRoleScope` and `revokeApiKey` are governed by the tenant on the row. Never use it instead of a scope check on a read: reads refuse by filtering, so it would answer `200 []` where a scope check answers 403 |
