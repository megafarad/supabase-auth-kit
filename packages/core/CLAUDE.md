...
The SQL side does the logging; this package supplies the two things SQL cannot see — the request, and the fact that a call was refused. `apps/installer/supabase/CLAUDE.md` has the model; what is here is the binding.

- **The request context is bound with the actor**, `kit.as(principalId, requestContext)`, and threaded onto every write as `p_request_ctx`. Same argument as the actor: a caller who passes it by hand will eventually pass the wrong one. `createAuthzContext` takes it from the adapter and hands it to `as`, so a request's rows carry its id without any call site knowing.
- **`RequestContext` is camelCase and becomes snake_case at the seam** (`requestContextToJson`), like every other argument. An empty context serialises to `undefined`, not `{}` — both transports omit an undefined argument, so the columns stay null rather than claiming there was a request nobody knows anything about.
- **Refusals are logged from here because they cannot be logged there.** A guard's `raise exception` rolls back its own transaction and any audit row in it, so `createWriteApi` catches `AuthzDeniedError`, writes a denied row naming what was attempted, and rethrows **the original error**. `enforceGuard` does the same for its two 403 branches, and for neither the 401 (nobody was refused anything) nor the 400 (a malformed request).
- **Audit writes never reject.** `logAudit` resolves to null on failure and reports through `audit.onError`. A logging failure must not become the caller's problem, least of all on a path already reporting a denial — the unit tests pin both halves: the original error propagates, and `onError` saw the real one.
- **`kit.pruneAuditLogs` is on the kit, not on `as(actor)`**, and the placement is the documentation: the SQL function takes no actor, because a retention job has no principal to name. It is the only database call in this package that is not made as somebody. It also does not swallow — unlike `logAudit`, a retention job that silently does nothing looks exactly like one that is keeping up — and the `{ deleted_count, lock_acquired }` pair exists because zero deleted means both "finished" and "another replica has it", which a loop must answer differently.
- **`audit.denials: false` turns off only the denial path**, which is the only part that costs a round trip that would not otherwise happen and the only part a caller can drive — a public endpoint behind a scope guard writes one row per probe. Successful writes log inside the transaction they are already in and cannot be turned off from here.
