...
The installer package's README covers [bootstrapping](apps/installer/README.md#bootstrap-the-first-administrator) and [the model](apps/installer/README.md#the-model) in full. Each adapter's README covers its options, guards and error responses.
...
An npm workspaces monorepo, built with [Turborepo](https://turborepo.com). Every package is ESM and TypeScript. `typescript`, `vitest` and `pg` are declared once, at the root.
...
The tests connect to `postgresql://postgres:postgres@127.0.0.1:54322/postgres` by default. Set `AUTHZ_TEST_DATABASE_URL` to use a different database.
...
There are two deliberate exceptions, both written by hand because `db diff` can't see them: the bootstrap migration, which inserts data, and the privileges migration, which sets grants. That has a consequence: **any migration that adds a function to `authz` must revoke `PUBLIC EXECUTE` on it by hand.** Postgres grants it to every new function, and `db diff` doesn't track privileges.
